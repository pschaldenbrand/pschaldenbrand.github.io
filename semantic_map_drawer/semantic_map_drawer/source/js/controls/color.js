DrawingBoard.Control.Color = DrawingBoard.Control.extend({
	...
	initTemplate: function() {
		...
		$.each([1], $.proxy(function(key, val) {
			...
			var nothing = this._rgba(0, 0, 0, 1);
			var sky = this._rgba(82, 232, 232, 1);
			var stone = this._rgba(123, 123, 123, 1);
			var tree = this._rgba(7, 119, 45, 1);
			var wood = this._rgba(119,56,7,1);
			var sea = this._rgba(15, 0, 225, 1);
			var rock = this._rgba(161, 161, 161, 1);
			var grass = this._rgba(0, 255, 26, 1);
			var sand = this._rgba(255, 239, 0, 1);
			var person = this._rgba(241, 8, 249, 1);
			var snow = this._rgba(231, 250, 250, 1);
			var water = this._rgba(0,186,223,1);
            var testing = this._rgba(255,0,0,1);

			var textures = [nothing, sky, stone, tree, wood, sea, rock, grass, sand, person, snow, water, testing];
			console.log(textures);
			textures.forEach(function(texture) {
				rainbows += DrawingBoard.Utils.tpl(oneColorTpl, {color: texture.toString()});});
			/*while (i <= 330) {
	...
	_rgba: function(r, g, b, a) {
		return { r: r, g: g, b: b, a: a, toString: function() { return "rgb(" + r +", " + g + ", " + b + ")"; } };
	},
